...
def compute_aolp(cos_aolp, sin_aolp):
    aolp = np.arctan2(sin_aolp, cos_aolp) / 2
...
mcubes_folder = '../MCubeS/'
count = 0
# Iterate through the files in the MCubeS folder
filenames = os.listdir(mcubes_folder + 'polL_dolp/')
...
for filename in sorted_filenames:
    ...
    if filename.endswith('.npy'):
        ...
        dolp_file = mcubes_folder + 'polL_dolp/' + filename
        dolp = load_and_inspect_npy(dolp_file)
        dolp = preprocess_dolp(dolp)

        cos_file = mcubes_folder + 'polL_aolp_cos/' + filename
        cos_aolp = load_and_inspect_npy(cos_file)

        sin_file = mcubes_folder + 'polL_aolp_sin/' + filename
        sin_aolp = load_and_inspect_npy(sin_file)

        color_image = cv2.imread(mcubes_folder + 'polL_color/' + filename.replace('.npy', '.png'))
