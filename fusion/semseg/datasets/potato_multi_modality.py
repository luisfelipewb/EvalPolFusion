...
class POTATOMM(Dataset):
    """
    num_classes: 2
    """
    CLASSES = ['background','bottle']

    PALETTE = torch.tensor([[ 0, 0, 0],
                [ 128,128, 128],
])# only use for visualization and inference

    def __init__(self, root: str = 'data/MCubeS/potato', split: str = 'train', transform = None, modals = ['image', 'aolp', 'dolp', 'dif', 'pol'], case = None) -> None:
        super().__init__()
        ...
        # print("NCLASSES POTATO: ",self.n_classes)
        # train_cfg = cfg['TRAIN']
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        dif = os.path.join(*[self.root, 'polL_dif', item_name+'.png'])
        pol = os.path.join(*[self.root, 'polL_pol', item_name+'.png'])
        ...
        _dif = cv2.imread(dif,-1)[:,:,::-1]
        _dif = _dif.astype(np.float32)/65535 if _dif.dtype==np.uint16 else _dif.astype(np.float32)/255
        _pol = cv2.imread(pol,-1)[:,:,::-1]
        _pol = _pol.astype(np.float32)/65535 if _pol.dtype==np.uint16 else _pol.astype(np.float32)/255
        ...
        # _img, _target, _aolp, _dolp = _img[:,self._left_offset:], _target[:,self._left_offset:], \
        #        _aolp[:,self._left_offset:], _dolp[:,self._left_offset:]

        sample = {'image': _img, 'label': _target, 'aolp': _aolp, 'dolp': _dolp, 'u_map': self.u_map, 'v_map': self.v_map, 'dif': _dif, 'pol': _pol}
        # sample = {'image': _img, 'label': _target , 'u_map': self.u_map, 'v_map': self.v_map}
    ...
    def transform_tr(self, sample):
        composed_transforms = transforms.Compose([
            ...
            RandomHorizontalFlip(),
            # FixedBandCropResizeSquare(440,256,256),
            FixedBandCropResizeSquare(384,512,512),
    ...
    def _get_file_names(self, split_name):
        ...
        source = os.path.join(self.root, 'list_folder', txt_file)
        file_names = []
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        dif = sample['dif']
        dif = np.array(dif).astype(np.float32)
        dif -= self.mean
        dif /= self.std

        pol = sample['pol']
        pol = np.array(pol).astype(np.float32)
        pol -= self.mean
        pol /= self.std
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        dif = np.array(dif).astype(np.float32).transpose((2, 0, 1))
        ...
        dif = torch.from_numpy(dif).float()
        pol = torch.from_numpy(pol).float()
...
class ResizeFixed(object):
    def __init__(self, size=(1024, 1224)):  # (H, W)
        self.size = size
    ...
    def __call__(self, sample):
        ...
        def resize(data):
            return cv2.resize(data, (w, h), interpolation=cv2.INTER_NEAREST)

        return {
            'image': resize(sample['image']),
            'label': resize(sample['label']),
            'dif': resize(sample['dif']),
            'pol': resize(sample['pol']),
            'aolp': resize(sample['aolp']),
            'dolp': resize(sample['dolp']),
            'u_map': resize(sample['u_map']),
            'v_map': resize(sample['v_map']),
        }

class FixedBandCropResizeSquare(object):
    def __init__(self, offset_y, crop_height, size=512):
    ...
    def __call__(self, sample):
        y1 = self.offset_y
        y2 = self.offset_y + self.crop_height

        def crop_and_resize(data, interp=cv2.INTER_NEAREST):
            cropped = data[y1:y2, :]
            return cv2.resize(cropped, (self.size, self.size), interpolation=interp)

        return {
            'image': crop_and_resize(sample['image'], interp=cv2.INTER_LINEAR),
            'label': crop_and_resize(sample['label'], interp=cv2.INTER_NEAREST),
            'dif': crop_and_resize(sample['dif'], interp=cv2.INTER_LINEAR),
            'pol': crop_and_resize(sample['pol'], interp=cv2.INTER_LINEAR),
            'aolp': crop_and_resize(sample['aolp'], interp=cv2.INTER_LINEAR),
            'dolp': crop_and_resize(sample['dolp'], interp=cv2.INTER_LINEAR),
            'u_map': crop_and_resize(sample['u_map'], interp=cv2.INTER_LINEAR),
            'v_map': crop_and_resize(sample['v_map'], interp=cv2.INTER_LINEAR),
        }

if __name__ == '__main__':
    traintransform = get_train_augmentation((1024, 1024), seg_fill=255)

    trainset = POTATO(transform=traintransform, split='val')
