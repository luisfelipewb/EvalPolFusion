...
# @torch.no_grad()
# def evaluate(model, dataloader, device,save_dir='predictions'):
#     print('Evaluating...')
#     model.eval()
...
@torch.no_grad()
def evaluate(model, dataloader, device, loss_fn=None, save_dir='predictions'):
    print('Evaluating...')
    model.eval()
    ...
    for images, labels,_ in tqdm(dataloader):
        ...
        if sliding:
            logits = sliding_predict(model, images, num_classes=n_classes)
        else:
            logits = model(images)
    ...
    return acc, macc, f1, mf1, ious, miou, val_loss
...
def main(cfg):
    ...
    for case in cases:
        ...
        msg = model.load_state_dict(torch.load(str(model_path), map_location='cpu',weights_only='True'))
        print(msg)
