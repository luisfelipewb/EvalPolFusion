...
class SemSeg:
    ...
    def predict(self, img_fname: str, overlay: bool) -> Tensor:
        if cfg['DATASET']['NAME'] == 'POTATO':
            lbl_path = img_fname.replace('/polL_color', '/GT')
        elif cfg['DATASET']['NAME'] == 'DELIVER':
            files = sorted(glob.glob(os.path.join(*[str(test_file), 'img', '*', 'val', '*', '*.png']))) # --- Deliver
        elif cfg['DATASET']['NAME'] == 'POTATOMM':
            # x1 = img_fname.replace('/polL_color', '/polL_aolp_cos').replace('.png', '.npy')
            # print("AAAAAAa")
            # print(f"modality aolpcos: {x1}")
            # x2 = img_fname.replace('/polL_color', '/polL_aolp_sin').replace('.png', '.npy')
            # x3 = img_fname.replace('/polL_color', '/polL_dolp').replace('.png', '.npy')
            # lbl_path = img_fname.replace('/polL_color', '/GT')

            x1 = img_fname.replace('/toval', '/polL_aolp_cos').replace('.png', '.npy')
            print("AAAAAAa")
            print(f"modality aolpcos: {x1}")
            x2 = img_fname.replace('/toval', '/polL_aolp_sin').replace('.png', '.npy')
            x3 = img_fname.replace('/toval', '/polL_dolp').replace('.png', '.npy')
            lbl_path = img_fname.replace('/toval', '/GT')
        ...
        # --- modals
        #commented for rgb modality only !!!!
        ...
        label[label==255] = 0
        label -= 1
        # not used for single modality
        sample = [img, x1, x2, x3][:len(modals)]
        # sample = [img][:len(modals)]
...
if __name__ == '__main__':
    ...
    # save_dir = Path(cfg['SAVE_DIR']) / 'preds' / (cfg['DATASET']['NAME']+'_'+cfg['MODEL']['BACKBONE']+'_'+modals_name)
    ...
    if test_file.is_file():
        ...
        if cfg['DATASET']['NAME'] == 'DELIVER':
            files = sorted(glob.glob(os.path.join(*[str(test_file), 'img', '*', 'val', '*', '*.png']))) # --- Deliver
        if cfg['DATASET']['NAME'] == 'POTATO':
            files = sorted(glob.glob(os.path.join(*[str(test_file),'*.png'])))

        if cfg['DATASET']['NAME'] == 'POTATOMM':
            files = sorted(glob.glob(os.path.join(*[str(test_file),'*.png'])))
        ...
        for file in files:
            ...
            print("segdone")
            filename = os.path.basename(file).replace('.png', '') + "_segMM.png"
            save_path = os.path.join("output/preds/RGBADval",filename)
